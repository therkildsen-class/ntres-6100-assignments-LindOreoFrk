...
Answer: I personally believe that plot 1.8 (SLR model for each region), 1.9 (density plot), 1.10 (histogram with facetting), 1.11 (boxplot with IQR) are the most informative ones. They provide information not just the data but also the trend and distribution.

The trend that I observe from these graphs is that, in western countries, the HDIs are higher meanwhile SSA has the lowest HDI which are associated with CPI - there is a positive relationship between two variables - one increases with the other one.
